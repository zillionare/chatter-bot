...
from chatter.feed.basefeed import BaseFeed
from chatter.web.accounts import Accounts
from chatter.web.interfaces import bp, ver
...
def start(port: int):
    ...
    ep = endpoint()
    logger.info("start chatter server at http://host:%s/%s", port, ep)
    bp.url_prefix = ep

    # added for gh://zillionare/chattering/issues/6
